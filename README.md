...
**Opção A — portátil (pronto):** descompacte `SideNotch-Medsystem-Setup-1.8.4.exe` em qualquer pasta e rode `SideNotch.exe`. Aparece um ícone na bandeja e a notch na borda direita da tela.
...
npm run dist
```
O instalador sai em `dist\SideNotch-Medsystem-Setup-1.8.4.exe` (requer Node.js 18+; no Windows não precisa de wine).
